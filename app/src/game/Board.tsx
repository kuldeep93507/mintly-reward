...
export const Board = memo(function Board() {
  ...
  return (
    <svg className="board-svg" viewBox="0 0 15 15" aria-label="Ludo board">
      ...
      {TRACK.map(([r, c], i) => {
        ...
        return (
          <rect key={i} x={c + 0.04} y={r + 0.04} width={0.92} height={0.92} rx={0.12}
            fill={sc ? PALETTE[sc].main : '#FFFFFF'} stroke="#D9D6EC" strokeWidth={0.05} />
        );
      })}
      {/* home columns */}
      ...
      {STAR_INDEXES.map((i) => {
        const [r, c] = TRACK[i];
        return <path key={i} d={STAR} transform={`translate(${c + 0.5} ${r + 0.5}) scale(1.25)`} fill="#C9C3E6" stroke="#9E97C9" strokeWidth={0.03} />;
      })}
      ...
      {ALL_COLORS.map((color) => {
        ...
        return (
          <g key={color}>
            ...
            <rect x={c0 + 0.9} y={r0 + 0.9} width={4.2} height={4.2} rx={0.55} fill="#FFFFFF" stroke={PALETTE[color].dark} strokeWidth={0.06} />
